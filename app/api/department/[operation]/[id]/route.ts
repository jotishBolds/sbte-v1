...
export async function GET(
  ...
  try {
    ...
    if (operation === "department") {
      ...
      if (!data) {
        return new NextResponse(JSON.stringify({ message: "Department not found" }), { status: 404 });
      }

      if (session.user.role !== "SBTE_ADMIN" && !data.isActive) {
        return new NextResponse(JSON.stringify({ message: "Department is not active" }), { status: 404 });
      }
      ...
      if (!data || data.length === 0) {
        return new NextResponse(JSON.stringify({ message: "No departments found for this college" }), { status: 404 });
      }
