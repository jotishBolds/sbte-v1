    ...
    try {
        ...
        const existingSubjectType = await prisma.subjectType.findFirst({
         ...
         if (existingSubjectType) {
            return NextResponse.json(
                { error: "Subject type with the same name or alias already exists for this college" },
                { status: 409 }
            );
        }
...
export async function GET(request: NextRequest) {
    try {
        ...
        // Find all SubjectTypes     for the specified college
        ...
        // Return the SubjectTypes with a success status of 200
