...
export async function PUT(
  ...
  try {
    ...
    const { termId, academicYearId, batchTypeId, startDate, endDate, status } =
    ...
    if (duplicateBatch) {
      return NextResponse.json(
        {
          error:
            "A batch with the same program, term, academic year, and batch type already exists",
        },
        { status: 400 }
      );
    }
