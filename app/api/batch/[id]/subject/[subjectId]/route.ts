//api/batch/[id]/subject/[subjectId]/route.ts
...
export async function PUT(
  ...
  try {
    ...
    if (subjectTypeId !== undefined && subjectTypeId !== "")
    ...
    return NextResponse.json(
      {
        message: "BatchSubject updated successfully",
...
export async function DELETE(
  ...
  try {
    ...
    if (
      !batchSubject ||
      batchSubject.batch.program.department.collegeId !== collegeId
    ) {
      return NextResponse.json(
        { error: "BatchSubject not found or you don't have access" },
        { status: 404 }
      );
    }
    ...
    return NextResponse.json(
      { message: "BatchSubject deleted successfully" },
      { status: 200 }
    );
  } catch (error) {
