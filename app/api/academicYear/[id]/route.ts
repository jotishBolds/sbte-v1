  ...
  try {
    ...
    if (data.startDate && !data.endDate) {
        ...
        if (data.startDate >= existingAcademicYear.endDate) {
          return NextResponse.json({ error: "Start date must be before the existing end date" }, { status: 400 });
        }
        ...
        if (existingAcademicYear.startDate >= data.endDate) {
          return NextResponse.json({ error: "End date must be after the existing start date" }, { status: 400 });
        }
      } else if (data.startDate && data.endDate) {
