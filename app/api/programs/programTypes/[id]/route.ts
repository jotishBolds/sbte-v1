//api/programs/programTypes/[id]/route.ts
  ...
  try {
    ...
    if (!id) {
      return NextResponse.json(
        { error: "Id of Program Type is not getting passed." },
        { status: 400 }
      );
    }
    ...
    if (existingProgramType) {
      return NextResponse.json(
        {
          error: `A program type with the name ${name} already exists in this college.`,
        },
        { status: 409 }
      );
    }
